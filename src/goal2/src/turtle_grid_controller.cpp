...
float kp=1.5,kp_theta=1.5,kp_y=0.5;
float kd=2,kd_theta=2,kd_y=1.5;
...
int main(int argc, char** argv)
{
    ...
    ros::Subscriber turtle_sub=n.subscribe("/turtle1/pose",100,turtleX_pose);
        ...
        //taking user input for the size of the grid 
         float width;
         cout<<"what width of the grid you would expect:";
         cin>>width;
	...
	//spawning the turtle to the starting point of the grid
...
//here we will impose the stopping condition for the turtle, when it reaches the nearby destination location             
    ...
    if(abs(err_pose_x)<0.05)
     {          velocity.linear.x=0; 
      ...
      break;
	      ...
	      //here we are the data of velocity of the turtle is continuously being published on the terminal  
...
while(ros::ok())
{
// this will keep the communication established between the  subscriber node and topic to get the msg callbacks
  while(itterator<=4)
       { ros::spinOnce();
        if(Pose.y>=11.0){break;}
         
         if(itterator==1)
           {
             //here we initiate the turtle to move only along the horizontal path of the grid 
             Pose.x=10.0;
             err_pose_x=Pose.x-(current_position.x);
             err_vel_x=0-(current_velocity.x);
             commanded_acc_x= ((current_velocity.x-pre_vel_x)/t_sampling)+ (kp*err_pose_x) + (kd*err_vel_x);
		...
		velocity.angular.y=0;
		velocity.angular.z=0;
               //here we define the stopping condition of the turtle
                if(abs(err_pose_x)<=0.05)
                  {velocity.linear.x=0;
                   itterator++;}
             ROS_INFO("%f,%f,%f,%f",velocity.linear.x,velocity.linear.y,velocity.linear.z,itterator);
             turtle_pub.publish(velocity);
             time_taken+=0; 
          }
       else if(itterator==2)
          {
           //here we initiate the turtle to move in vertical path of the grid 
           Pose.y= increamenter*width+1.0;
           err_pose_y=Pose.y-(current_position.y);
           err_vel_y=0-(current_velocity.y);
           commanded_acc_y= ((current_velocity.y-pre_vel_y)/t_sampling) + (kp_y*err_pose_y) + (kd_y*err_vel_y);
                velocity.linear.x= 0; 
		velocity.linear.y=commanded_acc_y*t_sampling;
		...
		velocity.angular.z=0;
          ...
          {
           // here we initiate the turtle  to move in the horizontal path
           Pose.x=1.0;
             err_pose_x=Pose.x-(current_position.x);
             err_vel_x=0-(current_velocity.x);
             commanded_acc_x= ((current_velocity.x-pre_vel_x)/t_sampling)+ (kp*err_pose_x) + (kd*err_vel_x);
...
increamenter=1;
cout<<"Turtle_reached at the goal";
  return 0;
}
